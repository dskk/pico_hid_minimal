...
// EP1 IN 送信 (エコーバック)
static void ep1_in_send(const uint8_t *data, uint16_t len) {
    ...
    usb_dpram->ep_buf_ctrl[1].in = len | USB_BUF_CTRL_AVAIL | USB_BUF_CTRL_FULL | pid;
}
    ...
    if (status & USB_INTS_SETUP_REQ_BITS) {
        ...
        if (request == USB_REQ_GET_DESCRIPTOR) {
            ...
            if (desc_type == DESC_TYPE_DEV) {
                ptr = usb_descs.dev.data;
                len = usb_descs.dev.len;
            } else if (desc_type == DESC_TYPE_CFG) {
                ptr = usb_descs.cfg.data;
                len = usb_descs.cfg.len;
            } else if (desc_type == DESC_TYPE_STR) {
                if (desc_idx < usb_descs.strings.len) {
                    ptr = usb_descs.strings.data[desc_idx];
                    len = ptr[0];
                }
            } else if (desc_type == DESC_TYPE_HID_REPORT) {
                ptr = usb_descs.report.data;
                len = usb_descs.report.len;
            }
